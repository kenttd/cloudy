...
export async function POST(req: any) {
  ...
  try {
    ...
    const storage_limit = user?.storage_limit;
    let used_storage = user?.used_storage;
    ...
    const uploadPromises = filesData.map(async (file: File) => {
      ...
      if (used_storage + file.size <= storage_limit) {
        ...
        used_storage = used_storage + file.size;
      } else {
    ...
    return NextResponse.json(
      {
        ...
        skip: Boolean(skipped_files.length != 0),
